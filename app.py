...
def classify(doc, pipeline, abstract, concrete, word2level):
    try:
        ...
        if doc._.language["language"] == "en":
            ...
            st.text(f"{difficulty[pipeline.predict(text_df)[0]]}")
        else:
...
nlp, abstract, concrete, word2level, pipeline = load_models_and_data(
    abstract_nouns_file=os.path.join("serialized", "wordlists", "abstract_nouns.pkl"),
    concrete_nouns_file=os.path.join("serialized", "wordlists", "concrete_nouns.pkl"),
    word2level_file=os.path.join("serialized", "wordlists", "word2level.json"),
    pipeline_file=os.path.join("serialized", "models", "all", "logistic_it.joblib"),
)
